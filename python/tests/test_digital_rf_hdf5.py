...
_dtype_strs = [
    's8', 'sc8', 'u8', 'uc8', 's16', 'sc16', 'u16', 'uc16',
    's32', 'sc32', 'u32', 'uc32', 's64', 'sc64', 'u64', 'uc64',
    'f32', 'fc32', 'f64', 'fc64',
]
_num_subchannels = [
    (1, 'x1'), (8, 'x8'),
]
_df_params = list(itertools.product(
    _dtype_strs, (sc[0] for sc in _num_subchannels),
))
_df_ids = list(''.join(p) for p in itertools.product(
    _dtype_strs, (sc[1] for sc in _num_subchannels),
))
...
def data_params(request):
    ...
    is_complex = (
        dtype.names is not None or np.issubdtype(dtype, np.complexfloating)
    )
...
@pytest.fixture(
    scope='session', params=[True, False], ids=['continuous', 'gapped'],
)
def file_params(request):
    return dict(
        is_continuous=request.param,
    )
...
def hdf_filter_params(request):
    return dict(
        compression_level=request.param[0],
        checksum=request.param[1],
    )
...
def end_global_index(
    ...
    samples_per_second, start_global_index, subdir_cadence_secs,
):
    ...
    nsamples_subdirs = int(
        (1.5 * subdir_cadence_secs * sample_rate_numerator)
        // sample_rate_denominator
    )
    nsamples_files = int(np.ceil(
        8 * file_cadence_millisecs * (samples_per_second / 1000)
    ))
...
@pytest.fixture(scope='session')
def bounds(
    end_global_index, file_cadence_millisecs, is_continuous,
    sample_rate_numerator, sample_rate_denominator, start_global_index,
):
    if is_continuous:
...
def data_block_slices(
    bounds, end_global_index, file_cadence_millisecs, samples_per_second,
    start_global_index,
):
...
def test_get_unix_time(
    sample_rate_numerator, sample_rate_denominator, start_datetime,
    start_global_index,
):
...
class TestDigitalRFChannel(object):
    ...
    def test_writer_init(
        ...
        for scs in (0, -10, 1.1):
            with pytest.raises(ValueError):
                drf_writer_factory(
                    directory=str(chdir), subdir_cadence_secs=scs,
                )
        ...
        for fcms in (0, -10, 1.1):
            with pytest.raises(ValueError):
                drf_writer_factory(
                    directory=str(chdir), file_cadence_millisecs=fcms,
                )
        ...
        with pytest.raises(ValueError):
            drf_writer_factory(
                directory=str(chdir), file_cadence_millisecs=1001,
        ...
        for srn in (0, -10, 1.1):
            with pytest.raises(ValueError):
                drf_writer_factory(
                    directory=str(chdir), sample_rate_numerator=srn,
                )
        ...
        for srd in (0, -10, 1.1):
            with pytest.raises(ValueError):
                drf_writer_factory(
                    directory=str(chdir), sample_rate_denominator=srd,
                )
        ...
        different_settings = [
            ...
            dict(subdir_cadence_secs=(
                ((p['subdir_cadence_secs'] * 1000)
                 // p['file_cadence_millisecs'] + 1)
    ...
    def test_writer_write_input_checking(
        self, channel, chdir, dtype, drf_writer_factory,
        ...
        chdir = chdir.dirpath().mkdir(
            '{0}_input_tests'.format(channel),
        )
        with drf_writer_factory(directory=str(chdir)) as dwo:
            # test interleaved input format (complex writer only)
            if is_complex:
                next_rel_index = dwo.rf_write(np.ones(
                    (shape[0], 2*shape[1]), dtype=dwo.realdtype,
                ))
            ...
            if is_complex:
                ...
                next_rel_index = dwo.rf_write(np.ones(
                    (shape[0], 2*shape[1]), dtype=base_type,
                ))
            ...
            with pytest.raises(ValueError):
                dwo.rf_write(
                    np.ones((shape[0], shape[1] + 1), dtype=dtype),
                )

        # test initializing with interleaved format
        if is_complex:
            chdir = chdir.dirpath().mkdir(
                '{0}_input_tests_interleaved'.format(channel),
            )
            ...
            with drf_writer_factory(
                ...
                dwo.rf_write(np.ones(
                    (shape[0], 2*shape[1]), dtype=realdtype,
                ))
    ...
    def test_writer_write_blocks(
        self, bounds, channel, chdir, data, data_block_slices,
        drf_writer_factory, start_global_index,
    ):
        """Test writer object's write_blocks method."""
        chdir = chdir.dirpath().mkdir(
            '{0}_write_blocks'.format(channel),
        )
        with drf_writer_factory(directory=str(chdir)) as dwo:
            ...
            block_sample_arr = np.concatenate(
                ([0], np.cumsum(block_lengths)[:-1]),
            )
            data_arr = np.concatenate(tuple(
                data[(bs - bounds[0]):(be - bounds[0])]
                for bs, be in zip(block_starts, block_stops)
            ))

            next_rel_index = dwo.rf_write_blocks(
                data_arr, global_sample_arr=global_sample_arr,
    ...
    @pytest.mark.firstonly('hdf_filter_params')
    def test_writer_output_files_write_blocks(
        self, channel, chdir, data_file_list,
    ):
        ...
        drf_files = digital_rf.lsdrf(
            str(chdir), include_drf=True, include_dmd=False,
    ...
    def test_writer_data_write(
        self, bounds, chdir, data, data_block_slices, data_file_list,
        drf_writer, start_global_index,
    ):
        ...
        for k, (sstart, sstop) in enumerate(data_block_slices):
            ...
            if rel_index == next_rel_index:
                ...
                next_rel_index = drf_writer.rf_write(
                    wdata, next_sample=rel_index,
                )
    ...
    def test_writer_get_last_file_written(
    ...
    def test_reader_get_properties(
        self, bounds, channel, drf_reader, drf_writer_param_dict,
        samples_per_second, start_datetime, start_global_index,
    ):
        ...
        sample_props = drf_reader.get_properties(
            channel, sample=start_global_index,
        )
        ...
        with pytest.raises(IOError):
            drf_reader.get_properties(
                channel, sample=bounds[0] - 1,
            )
        with pytest.raises(IOError):
            drf_reader.get_properties(
                channel, sample=bounds[1] + 1,
            )

    @pytest.mark.firstonly(
        'data_params', 'file_params', 'hdf_filter_params',
    )
    def test_reader_get_digital_metadata(
        self, channel, chdir, drf_reader, sample_params,
        start_global_index,
    ):
        ...
        dmd_writer = digital_rf.DigitalMetadataWriter(
            ...
            file_cadence_secs=1,
            sample_rate_numerator=(
                sample_params['sample_rate_numerator']
            ),
            sample_rate_denominator=(
                sample_params['sample_rate_denominator']
            ),
            file_name='metadata',
        )
    ...
    def test_reader_read_metadata(
        self, bounds, channel, drf_reader, samples_per_second,
        sample_rate_numerator, sample_rate_denominator, start_global_index,
    ):
        ...
        md = drf_reader.read_metadata(
            bounds[0], bounds[1], block_channel, method=None,
        )
        ...
        md = drf_reader.read_metadata(
            bounds[0], bounds[1], channel, method=None,
        )
    ...
    def test_reader_get_last_write(
        self, channel, chdir, data_file_list, drf_reader,
    ):
    ...
    def test_reader_get_continuous_blocks(
        ...
        # fail when channel doesn't exist
        with pytest.raises(KeyError):
            drf_reader.get_continuous_blocks(
                bounds[0], bounds[1], 'not_a_channel',
            )

    def test_reader_read(
        self, bounds, channel, data, data_block_slices, drf_reader,
        num_subchannels,
    ):
        ...
        with pytest.raises(ValueError):
            drf_reader.read(
                bounds[0], bounds[1], channel, sub_channel=num_subchannels,
            )
    ...
    @pytest.mark.firstonly(
        'data_params', 'hdf_filter_params', 'sample_params',
    )
    def test_reader_multiple_topleveldirs(
        ...
        writers = [
            drf_writer_factory(directory=str(tld.mkdir(channel)))
            for tld in tlds
        ]
        for dwo, (sstart, sstop) in zip(
            itertools.cycle(writers), data_block_slices,
        ):
            rel_index = sstart - start_global_index
            bstart = sstart - bounds[0]
            bstop = sstop - bounds[0]
