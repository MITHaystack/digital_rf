classdef DigitalRFReader
    ...
    methods
        function reader = DigitalRFReader(topLevelDirectories)
            ...
            dirFlag = 0;
            for i = 1:dims(1)
                topGlobPath = fullfile(strtrim(reader.topLevelDirectories(i,:)),'*', 'metadata.h5');
            ...
            for i = 1:uniqueDims(1)
                ...
                for j = 1:chanDims(2)
                    ...
                    if strcmp(thisItem, thisChannel)
                        ...
                        metaFile = fullfile(this_top_level, thisChannel, 'metadata.h5');
                        this_subdir_cadence_secs = h5readatt(metaFile, '/', 'subdir_cadence_secs');
                        this_file_cadence_millisecs = h5readatt(metaFile, '/', 'file_cadence_millisecs');
                        this_samples_per_second_numerator = h5readatt(metaFile, '/', 'sample_rate_numerator');
                        this_samples_per_second_denominator = h5readatt(metaFile, '/', 'sample_rate_denominator');
                        this_is_complex = h5readatt(metaFile, '/', 'is_complex');
                        this_num_subchannels = h5readatt(metaFile, '/', 'num_subchannels');
